...
+ Your submission **must** compile successfully with a single `make` command. You can update the provided [Makefile](sampleakefile) to compile your code. The produced executable file **must** be named `ssi`. Otherwise, you will lose marks.
...
A sample [Makefile](sampleakefile) is provided to help you build the sample code. It takes care of linking-in the GNU readline library for you. The sample code shows you how to use `readline()` to get input from the user, only if you choose to use the readline library.

### 7.2 Helper Programs

#### 7.2.1 [inf.c](samplenf.c)
